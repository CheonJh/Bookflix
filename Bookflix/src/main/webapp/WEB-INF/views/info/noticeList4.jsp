      ...
      <ul class="pagination">
        <c:if test="${prev}">
          <li class="page-item"><a class="page-link" id="page-link"
            href="/info/noticeList4?num=${startPageNum -1}">이전</a></li>
        </c:if>

        <c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
          <c:if test="${select != num}">
            <li class="page-item"><a class="page-link"
              id="page-link" href="/info/noticeList4?num=${num}">${num}</a></li>
          </c:if>
          <c:if test="${select == num}">
        ...
        <c:if test="${next}">
          <li class="page-item"><a class="page-link" id="page-link"
            href="/info/noticeList4?num=${endPageNum +1}">다음</a></li>
        </c:if>
      </ul>
    </div>
  </div>
