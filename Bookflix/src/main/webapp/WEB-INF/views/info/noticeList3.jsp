...
<body>
  ...
  <div class="ntcontainer">
    <table class="table table-bordered tbFAQ">
      ...
      <c:forEach items="${notiList3}" var="notiList3">
        <tr>
          <td>${notiList3.notice_num}</td>
          <td>${notiList3.notice_category}</td>
          <td><a
            href="/info/viewCnt?notice_num=${notiList3.notice_num}">${notiList3.notice_title}</a></td>
          <td><c:out value="${notiList3.notice_viewcnt}" /></td>
          <td><fmt:formatDate value="${notiList3.notice_date}"
      ...
      <ul class="pagination">
        <c:if test="${prev}">
          <li class="page-item"><a class="page-link" id="page-link"
            href="/info/noticeList3?num=${startPageNum -1}">이전</a></li>
        </c:if>

        <c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
          <c:if test="${select != num}">
            <li class="page-item"><a class="page-link"
              id="page-link" href="/info/noticeList3?num=${num}">${num}</a></li>
          </c:if>
          <c:if test="${select == num}">
        ...
        <c:if test="${next}">
          <li class="page-item"><a class="page-link" id="page-link"
            href="/info/noticeList3?num=${endPageNum +1}">다음</a></li>
        </c:if>
      </ul>
    </div>
  </div>
